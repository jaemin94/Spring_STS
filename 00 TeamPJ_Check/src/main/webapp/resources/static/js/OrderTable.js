...
function getSelectedOrderIds() {
  ...
  const orderIds = Array.from(selectedOrders).map((order) => {
...
function sendData() {
  ...
  selectedOrders.forEach((order) => {
    const order_id = order.querySelector("[name='order_id']").textContent;
    const member_id = order.querySelector("[name^='member_id']").textContent;
    const odr_amount = order.querySelector("[name^='odr_amount']").textContent;
    const price = order.querySelector("[name^='price']").textContent;
	
	console.log(order_id);

    if (!order_id || !member_id || isNaN(parseInt(odr_amount)) || isNaN(parseInt(price))) {
    ...
    const orderData = {
      "order_id": order_id,
      "member_id": member_id,
      "odr_amount": parseInt(odr_amount),
  ...
  axios.put('/app/order/Order_Update', orderUpdateRequest)
    .then(response => {
...
const orderIdsString = JSON.stringify(selectedOrderIds);
const firstOrderId = selectedOrderIds[0];
  axios.delete(`/app/order/Order_delete`, { params: { order_id: firstOrderId } })
    .then(response => {
