...
public class WebMvcConfig implements WebMvcConfigurer{
	...
	@Bean("/custom01")
	public CustomHandler handerTest1()
	{
		return new CustomHandler();
	}
	
	// SimpleUrlHandlerMapping 	: 개발자가 직접 매핑정보를 설정, 점적자원에 대한 매핑정보를 설정 기본값
	@Bean
	...
	RequestMappingHandlerMapping requestMappingHandlerMapping() throws NoSuchMethodException, SecurityException
	{
		...
		// Reflection 으로 Method찾기
//		Method[] methods = CustomHandler.class.getDeclaredMethods();
//		System.out.println("METHODS[0] " + methods[0].getName());
//		System.out.println("METHODS[1] " + methods[1].getName());
		...
		RequestMappingInfo mappingInfo = RequestMappingInfo.paths("/custom03")
										 .methods(RequestMethod.GET)  // 웹요청이 없을시 생략 가능
										 .build();
